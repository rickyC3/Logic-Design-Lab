set_property IOSTANDARD LVCMOS33 [get_ports {D[7]}]
set_property IOSTANDARD LVCMOS33 [get_ports {D[6]}]
set_property IOSTANDARD LVCMOS33 [get_ports {D[5]}]
set_property IOSTANDARD LVCMOS33 [get_ports {D[4]}]
set_property IOSTANDARD LVCMOS33 [get_ports {D[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {D[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {D[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {D[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ssd[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports caps_state]
set_property IOSTANDARD LVCMOS33 [get_ports clk_100Hz]
set_property IOSTANDARD LVCMOS33 [get_ports ps2_clk]
set_property IOSTANDARD LVCMOS33 [get_ports ps2_data]
set_property IOSTANDARD LVCMOS33 [get_ports rst_p]
set_property PACKAGE_PIN U16 [get_ports caps_state]
set_property PACKAGE_PIN W5 [get_ports clk_100Hz]
set_property PACKAGE_PIN C17 [get_ports ps2_clk]
set_property PACKAGE_PIN B17 [get_ports ps2_data]
set_property PACKAGE_PIN R2 [get_ports rst_p]
set_property PACKAGE_PIN W4 [get_ports {ssd[3]}]
set_property PACKAGE_PIN V4 [get_ports {ssd[2]}]
set_property PACKAGE_PIN U4 [get_ports {ssd[1]}]
set_property PACKAGE_PIN U2 [get_ports {ssd[0]}]
set_property PACKAGE_PIN W7 [get_ports {D[7]}]
set_property PACKAGE_PIN W6 [get_ports {D[6]}]
set_property PACKAGE_PIN U8 [get_ports {D[5]}]
set_property PACKAGE_PIN V8 [get_ports {D[4]}]
set_property PACKAGE_PIN U5 [get_ports {D[3]}]
set_property PACKAGE_PIN V5 [get_ports {D[2]}]
set_property PACKAGE_PIN U7 [get_ports {D[1]}]
set_property PACKAGE_PIN V7 [get_ports {D[0]}]

set_property IOSTANDARD LVCMOS33 [get_ports {ascii[6]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ascii[5]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ascii[4]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ascii[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ascii[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ascii[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ascii[0]}]
set_property PACKAGE_PIN L1 [get_ports {ascii[6]}]
set_property PACKAGE_PIN P1 [get_ports {ascii[5]}]
set_property PACKAGE_PIN N3 [get_ports {ascii[4]}]
set_property PACKAGE_PIN P3 [get_ports {ascii[3]}]
set_property PACKAGE_PIN U3 [get_ports {ascii[2]}]
set_property PACKAGE_PIN W3 [get_ports {ascii[1]}]
set_property PACKAGE_PIN V3 [get_ports {ascii[0]}]
